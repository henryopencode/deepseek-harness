...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-speech-input': { kind: 'none', reason: 'Browser-side dictation control; recognized text stays in the human-owned draft until ordinary composer submission.' },
  ...
  'packages/speech/speech-to-text-local': { kind: 'none', reason: 'Host-local transcription returns text to a human-owned draft and never appends a Session event or model message.' },
