...
export const zh = {
  ...
  'drop.title': '文件拖动到此处即可添加',
  'drop.desc': '最多 {count} 张图片，每张 {size}；其他文件将插入其本地路径',
  'drop.blocked': '当前无法添加文件',
  'file.pathUnavailable': '无法读取 {count} 个文件的本地路径（浏览器限制）',
  'file.pathUnavailableNamed': '无法读取 {name} 的本地路径（浏览器限制）',
...
export const en = {
  ...
  'drop.title': 'Drag files here to add them',
  'drop.desc': 'Up to {count} images, {size} each; other files insert their local path',
  'drop.blocked': 'Files cannot be added right now',
  'file.pathUnavailable': "Couldn't read the local path of {count} dropped files (browser restriction)",
  'file.pathUnavailableNamed': "Couldn't read the local path of {name} (browser restriction)",
